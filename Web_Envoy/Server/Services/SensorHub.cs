...
namespace Server
{
    public class SensorHub : SensorService.SensorServiceBase
    {
        ...
        {
            _logger.LogInformation($"received a GetAvailableSensors request from user {request.Username} message: {request.Message}");
        ...
        public override async Task ReceiveTemperatureUpdates(TemperatureRequest request, IServerStreamWriter<TemperatureData> responseStream, ServerCallContext context)
        {
            ...
            while (!context.CancellationToken.IsCancellationRequested)
            {
                ...
                await responseStream.WriteAsync(new TemperatureData { Devicelocation = $"Random location {randomizer.Next(1, 100)}", Temperature = randomizer.Next(1, 100) });
