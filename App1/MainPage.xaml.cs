...
namespace App1
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void StartCapturePreview_Click(object sender, RoutedEventArgs e)
        {
        ...
        private async Task StartClient()
        {
            ...
            var channel = new Channel("grpcservices.softwarelab.it", 12345, ChannelCredentials.Insecure);
            ...
            using (var call = client.StreamVideo())
            {
                    ...
                    while (await call.ResponseStream.MoveNext())
                    {
                        try
                        {
                            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, async () =>
                            {
                                ...
                                {
                                    responseStreamBytes = new byte[call.ResponseStream.Current.Frame.Length];
                                    call.ResponseStream.Current.Frame.CopyTo(responseStreamBytes, 0);
                                    await stream1.WriteAsync(responseStreamBytes.AsBuffer());
                                    stream1.Seek(0);
                                    await source.SetSourceAsync(stream1);
                                }
                                
                                ServerPreview.Source = source;
                                UpdateFPS();
                                ServerPreviewQuality.Text = $"Image pixel quality {source.PixelWidth} X {source.PixelHeight}";
                                ServerPreviewFileSize.Text = $"File Size: {responseStreamBytes.Length} bytes";
                            });
                        }
                        catch(Exception ex)
                        {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        frame.Frame = Google.Protobuf.ByteString.CopyFrom(await GetCameraFrame());
        ...
        void UpdateFPS()
        {
            ...
            ServerPreviewFPS.Text = $"{_fps} FPS";
        }
        private async Task<byte[]> GetCameraFrame()
        {
            byte[] bytes;
            try
            {
                ...
                using (var currentFrame = await _mediaCapture.GetPreviewFrameAsync(videoFrame))
                {
                    ...
                    using (var outputStream = new InMemoryRandomAccessStream())
                    {
                        ...
                        bytes = new byte[outputStream.Size];
